...
function formatStatus(value) {
  ...
  if (["YES", "PAID"].includes(normalized)) return "Paid";
  if (["NOT_REQUIRED", "N/A", "NA"].includes(normalized)) return "Not required";
  if (["NO", "UNPAID", "PENDING"].includes(normalized)) return "Unpaid";
...
export function mapAuthToProfile({ user, phone, email } = {}) {
  ...
  return {
    ...
    dateJoined: formatDateJoined(user?.date_joined_organization || user?.created_at),
    dateJoinedOrganization: user?.date_joined_organization || "",
    pastPositions: toList(user?.past_positions),
    updatedAt: user?.updated_at || null,
    email: user?.email || email || "—",
    phone: user?.phone_number || phone || "—",
    whatsapp: user?.whatsapp_number || user?.phone_number || phone || "—",
    avatar: user?.profile_picture_url || null,
    membershipType: formatTitle(user?.membership_type),
    position: user?.position || "Member",
    affiliationStatus: formatStatus(user?.year_affiliation_paid_status),
    duesStatus:
      user?.branch_collects_dues === false
        ? "Not required"
        : formatStatus(user?.month_dues_paid_status),
    collectsDues: user?.branch_collects_dues !== false,
